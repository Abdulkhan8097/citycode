...
class ChatModel extends Model{
     ...
     protected $allowedFields = ['id','category_id','chat_type','send_by','group_id','list_id','sender_id','receiver_id','msg','link','image','video','video_thumb','doc','caption','lat','long','type','file_size','seen_status','app_db_id','created_date','date_string','new_date_created','msg_id','user_chat_status'];
	...
	public function getData($searchArray=array(), $offset='', $limit='',$coutOnly='',$showQuery='')
    {
        ...
        else
        {
            ...
            $sql = "SELECT ad.*, IF(ad.send_by='branch',(SELECT branch_name FROM branch where branch.branch_id=ad.sender_id),(SELECT branch_name FROM branch where branch.branch_id=ad.receiver_id)) AS branch_name , ";
            $sql .= " IF(ad.send_by='company',(SELECT company_name FROM company_details where company_details.id=ad.sender_id),(SELECT company_name FROM company_details where company_details.id=ad.receiver_id)) AS company_name , ";
            $sql .= " IF(ad.send_by='customer',(SELECT name FROM customer where customer.id=ad.sender_id),(SELECT name FROM customer where customer.id=ad.receiver_id)) AS customer_name  ";
     ...
     public function getChatData($searchArray=array(), $offset='', $limit='',$coutOnly='',$showQuery='')
    {
            ...
            $sql = "SELECT ad.*, B.branch_name,C.name, COM.company_name ";
            $sql .= " FROM $this->table AS ad ";
            $sql .= " LEFT JOIN branch B ON (ad.sender_id=B.branch_id) ";
            $sql .= " LEFT JOIN company_details COM ON (COM.id=B.company_id) ";
           ...
           if ((isset($searchArray['company_id'])) && (isset($searchArray['company_id']))) {
            $sql .= " AND B.company_id= '".$searchArray['id']."' ";
           }
