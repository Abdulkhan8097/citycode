...
class VipCustomerModel extends Model {

    protected $table = 'vip_customer';
    protected $primaryKey = 'id';
    protected $allowedFields = ['id', 'company_id', 'customer_id', 'created_date'];
    ...
    public function getData($searchArray = array(), $offset = '', $limit = '', $coutOnly = '') {
        if ($coutOnly) {
            ...
            $sql = "SELECT ad.*, C.company_name FROM $this->table AS ad ";
        }
    ...
    public function vip_code($code) {
        $sql = "SELECT company_id,id FROM $this->table";
        $sql .= " WHERE company_id ='" . $code . "' ";
        $query = $this->db->query($sql);
        $result = $query->getRow();
        return $result;
    }
...
/*public function getVip($id) {
      ...
      $sql = "SELECT vip_customer.* FROM $this->table";
