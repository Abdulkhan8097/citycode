...
class CompanyreportController extends BaseController {
    ...
    function index() {
        ...
        $data['totalcommision'] = $orderModel->getData($searchArray, $startLimit, $Limit);

        //------count customer & Comapny Active/ Inactive
        ...
        // $data ['customer_status'] = $userModel->countActiveCustomer();
        // $data["active_customer_count"]=$data["customer_status"]->ActiveCustomer;
        //$data ['active_customer_count'] = $userModel->countActiveCustomer();
        //print_r($data ['active_customer_count'][0]); exit;
        //-------Active Company
        $compModel = new CompanyModel();
        $searchArray = '';
        $data["companycount"] = $compModel->getDataAll($searchArray, '', '', 1);
        $searchArray = array('status' => 1);
        $data["activecompanycount"] = $compModel->getDataAll($searchArray, '', '', 1);
//        $data["companystatus"] = $compModel->countActiveCompany();
//        $data["activecompanycount"]=$data["companystatus"]->ActiveCompany;
        //echo $data["activecompanycount"];exit;
        //print_r($data["companystatus"]->ActiveCompany);exit;
        //-------------------------------
    ...
    function admincompanyreport() {
        ...
        $stateid = $this->request->getGet('stateid');
        $cityid = $this->request->getGet('cityid');
        $company_name = $this->request->getGet('company_id');
        $coupon_type = $this->request->getGet('coupon_type');
        ...
        if($company_name) {
            $searchArray['company_id'] = $company_name;
        }
        if($coupon_type){
            $searchArray['coupon_type'] = $coupon_type;
        }
